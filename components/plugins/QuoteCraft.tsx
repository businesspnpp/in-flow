...
export default function QuoteCraft({
  activeChat,
  aiContext,
  aiPrefill,
}: QuoteCraftProps) {
  ...
  async function sendQuote() {
    ...
    const { error: messageError } = await supabase
      .from("messages")
      .insert({ chat_id: activeChat.id, sender: "business", body });
    if (messageError) {
    ...
    await supabase
      .from("chats")
      .update({
        last_message: `Quote ${reference}: R${subtotal.toFixed(2)}`,
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input
                  ...
                  className="bg-white border border-zinc-300 px-4 py-2.5 text-sm text-zinc-900 placeholder-zinc-400 outline-none focus:border-zinc-500"
                  placeholder="Qty"
                />{" "}
                <input
                  type="number"
        ...
        <div className="absolute inset-0 bg-white border border-zinc-200 p-5 flex flex-col gap-3 z-20">
          ...
          <div className="max-h-56 overflow-y-auto border border-zinc-200 bg-zinc-50 p-5 text-xs text-zinc-900 space-y-2">
            ...
            {selectedRows.map((row) => (
              <div key={row.item.id} className="flex justify-between gap-3">
                {" "}
                <span>
                  {row.item.name} x{row.state.quantity}
                </span>{" "}
          ...
          <div className="flex gap-2 mt-auto">
            ...
            <button
              ...
              className="flex-1 border border-zinc-300 bg-white py-2.5 text-sm font-semibold text-zinc-900 hover:bg-zinc-50 transition-colors"
            >
              {" "}
              Back{" "}
            </button>{" "}
