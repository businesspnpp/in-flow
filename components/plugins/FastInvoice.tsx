...
interface AiCandidateItem { item: string; quantity?: number; price?: number; }
...
function parseAiCandidates(payload: unknown): AiCandidateItem[] {
  ...
  if (Array.isArray(lineItems)) {
    lineItems.forEach((entry) => {
      ...
      output.push({ item: row.item, quantity: typeof row.quantity === 'number' ? row.quantity : 1, price: typeof row.price === 'number' ? row.price : undefined });
    });
  }
...
export default function FastInvoice({ activeChat, aiContext, aiPrefill }: FastInvoiceProps) {
  ...
  useEffect(() => {
    async function loadCatalog() {
      ...
      setBusinessId(id);
      const { data, error: catalogError } = await supabase.from('inflow_items_catalog').select('*').eq('business_id', id).eq('is_active', true).order('sort_order', { ascending: true });
      ...
      rows.forEach((item) => { baseline[item.id] = { checked: false, quantity: 1, unitPrice: Number(item.price) }; });
      setDraft(baseline);
      setLoading(false);
    }
    loadCatalog();
  ...
  useEffect(() => {
    ...
    if (candidates.length === 0) {
      ...
      setDraft((prev) => {
        const next = { ...prev };
        catalog.forEach((item) => { if (textBlob.includes(item.name.toLowerCase())) next[item.id] = { ...next[item.id], checked: true }; });
        return next;
      });
      return;
    }
  ...
  const selectedRows = catalog.map((item) => ({ item, state: draft[item.id] })).filter((row) => row.state?.checked);
  ...
  async function sendInvoice() {
    ...
    const lineItems = selectedRows.map((row) => ({ catalogItemId: row.item.id, item: row.item.name, description: row.item.description, price: row.state.unitPrice, quantity: row.state.quantity }));
    const chatId = isUuid(activeChat.id) ? activeChat.id : null;

    if (!invoiceTableUnavailable) {
      const { error: insertInvoiceError } = await supabase.from('inflow_invoices').insert({ business_id: businessId, chat_id: chatId, type: 'invoice', reference, customer_name: activeChat.name, line_items: lineItems, subtotal, vat_amount: 0, total: subtotal, currency: 'ZAR', status: 'sent' });
      if (insertInvoiceError && !isMissingTableError(insertInvoiceError, 'inflow_invoices')) { setError(insertInvoiceError.message); setSending(false); return; }
      if (insertInvoiceError && isMissingTableError(insertInvoiceError, 'inflow_invoices')) invoiceTableUnavailable = true;
    }

    const messageLines = selectedRows.map((row) => `• ${row.item.name} x${row.state.quantity} — R${(row.state.quantity * row.state.unitPrice).toFixed(2)}`).join('\n');
    const invoiceText = `📄 *Invoice ${reference}*\n━━━━━━━━━━━━━━━━━\n${messageLines}\n━━━━━━━━━━━━━━━━━\nTotal: R${subtotal.toFixed(2)}\nPay now: ${invoiceLink}`;

    await ensureChatExists(activeChat.id, { name: activeChat.name, lastMessage: invoiceText });
    const { error: messageError } = await supabase.from('messages').insert({ chat_id: activeChat.id, sender: 'business', body: invoiceText });
    if (messageError) { setError(messageError.message); setSending(false); return; }
    await supabase.from('chats').update({ last_message: `Invoice ${reference}: R${subtotal.toFixed(2)}`, updated_at: new Date().toISOString() }).eq('id', activeChat.id);
  ...
  return (
    <div className="flex flex-col gap-4 relative">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-zinc-800">Fast Invoice</h3>
      ...
      {!activeChat && <p className="text-xs text-zinc-500 text-center py-4">Select a chat to send an invoice</p>}
      {catalog.length === 0 && <p className="text-xs text-zinc-500">No active catalog items. Add items in Menu Drop first.</p>}
              ...
              <div key={item.id} className="border border-zinc-200 bg-white p-3 flex flex-col gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    onChange={(e) => patchDraft(item.id, { quantity: Math.max(1, Number.parseInt(e.target.value || '1', 10)) })}
                  ...
                  <input
                    ...
                    onChange={(e) => patchDraft(item.id, { unitPrice: Math.max(0, Number.parseFloat(e.target.value || '0')) })}
                    className="border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:border-zinc-500"
      ...
      <div className="border border-zinc-200 bg-zinc-50 p-3 text-sm">
        <div className="flex items-center justify-between">
          ...
          <span className="font-medium text-zinc-800">{selectedRows.length} item{selectedRows.length !== 1 ? 's' : ''}</span>
        </div>
        <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-zinc-200">
          <span className="text-zinc-500">Total</span>
          <span className="font-semibold text-zinc-900">R{subtotal.toFixed(2)}</span>
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="absolute inset-0 bg-white border border-zinc-200 p-4 flex flex-col gap-3 z-20">
          <h4 className="text-sm font-semibold text-zinc-900">Confirm invoice</h4>
          <div className="max-h-56 overflow-y-auto border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-700 space-y-2">
            {selectedRows.map((row) => (
              <div key={row.item.id} className="flex justify-between gap-3">
                <span>{row.item.name} ×{row.state.quantity}</span>
          ...
          <div className="flex gap-2 mt-auto">
            <button
              ...
              className="flex-1 border border-zinc-300 py-2 text-sm text-zinc-700 hover:bg-zinc-50 transition-colors"
            >
              Back
            </button>
            <button
