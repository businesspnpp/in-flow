...
export default function ChatsContent() {
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden h-full bg-slate-50">
      {/* Conversation list */}
      <div className={`flex-shrink-0 flex flex-col bg-white border-r border-zinc-200 overflow-hidden ${activeContact ? 'hidden md:flex' : 'flex'} w-full md:w-72 lg:w-80`}>
        <div className="px-4 pt-5 pb-3">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold tracking-tight text-zinc-900">Inbox</h2>
              <p className="text-[11px] text-zinc-500 mt-0.5">{MOCK_CONVERSATIONS.length} conversations</p>
            </div>
            <div className="flex items-center gap-1">
              <button className="h-8 w-8 flex items-center justify-center rounded-lg text-zinc-500 hover:bg-blue-50 hover:text-blue-600 transition"><Hash size={16} strokeWidth={2.25} /></button>
              <button className="h-8 w-8 flex items-center justify-center rounded-lg text-zinc-500 hover:bg-blue-50 hover:text-blue-600 transition"><Users size={16} strokeWidth={2.25} /></button>
            </div>
          </div>
          <div className="relative">
            <Search size={16} strokeWidth={2.25} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search conversations…" className="w-full h-10 rounded-lg border border-zinc-200 bg-zinc-50 pl-9 pr-3 py-2.5 text-xs text-zinc-700 placeholder:text-zinc-400 outline-none focus:border-blue-400 focus:bg-white transition" />
          </div>
          <div className="flex gap-1.5 mt-3">
            {['All', 'Unread', 'Mine'].map(f => (
              <button key={f} className={`px-3 py-1.5 rounded-full text-[11px] font-semibold transition ${f === 'All' ? 'bg-blue-600 text-white shadow-sm' : 'bg-zinc-100 border border-zinc-200 text-zinc-500 hover:text-zinc-800'}`}>{f}</button>
            ))}
          </div>
        </div>
        <div className="flex-1 overflow-y-auto divide-y divide-zinc-100">
          {filtered.map(contact => (
            <button key={contact.id} onClick={() => setActiveContact(contact.id)} className={`w-full flex items-start gap-3 px-4 py-3.5 text-left transition-colors hover:bg-blue-50/60 ${activeContact === contact.id ? 'bg-blue-50/80' : ''}`}>
              <div className="relative flex-shrink-0">
                <div className={`h-10 w-10 rounded-full flex items-center justify-center text-[13px] font-semibold text-white shadow-sm ${CHANNEL_COLORS[contact.channel] || 'bg-zinc-600'}`}>{contact.avatarColor}</div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {contact.statusTag && <span className={`mt-1.5 inline-block rounded-full border px-2 py-0.5 text-[10px] font-medium ${contact.statusColor}`}>{contact.statusTag}</span>}
              </div>
      ...
      {/* Thread */}
      <div className={`flex-1 flex flex-col overflow-hidden bg-slate-50 ${!activeContact ? 'hidden md:flex' : 'flex'}`}>
        {!activeContact ? (
          ...
          <>
            <div className="flex-shrink-0 flex items-center gap-3 px-4 py-3 border-b border-zinc-200 bg-white">
              <button onClick={() => setActiveContact(null)} className="md:hidden flex h-8 w-8 items-center justify-center rounded-lg bg-zinc-100 text-zinc-500 transition"><ArrowLeft size={16} /></button>
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setShowIntel(true)} className="xl:hidden flex items-center gap-1.5 h-8 px-2.5 rounded-lg text-xs font-medium bg-zinc-100 text-zinc-700 border border-zinc-200 transition-colors"><Users size={13} /> Intel</button>
                <button onClick={handleAi} disabled={aiLoading} className={`flex items-center gap-1.5 h-8 px-2.5 rounded-lg text-xs font-medium transition-colors border ${aiLoading ? 'bg-blue-50 text-blue-700 border-blue-200 animate-pulse cursor-wait' : 'bg-blue-50 text-blue-700 hover:bg-blue-100 border-blue-200'}`}><Sparkles size={13} />{aiLoading ? 'Reading…' : 'AI Assist'}</button>
                <button className="h-8 w-8 flex items-center justify-center rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 transition"><Phone size={15} /></button>
                <button className="h-8 w-8 flex items-center justify-center rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 transition"><Star size={15} /></button>
            ...
            {aiExtraction?.tool && (
              <div className="flex-shrink-0 flex items-center gap-3 px-4 py-2.5 bg-blue-50 border-b border-blue-100">
      ...
      {showIntel && selected && (
        <div className="xl:hidden fixed inset-0 z-[70] bg-zinc-900/40">
          <div className="absolute inset-x-0 bottom-0 max-h-[86vh] rounded-t-2xl border-t border-zinc-200 bg-white flex flex-col">
            <div className="px-4 py-3 border-b border-zinc-200 flex items-center justify-between">
              ...
              <button onClick={() => setShowIntel(false)} className="h-8 w-8 rounded-lg border border-zinc-300 text-zinc-700 hover:bg-zinc-100 transition"><X size={14} className="mx-auto" /></button>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              ...
              <button onClick={() => { setShowIntel(false); router.push('/dashboard/tools'); }} className="w-full px-3 py-2.5 rounded-lg text-sm font-semibold bg-blue-600 text-white">Open Tools</button>
            </div>
          </div>
