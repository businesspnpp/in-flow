                ...
                <tbody className="divide-y divide-zinc-100">
                  {filteredCustomers.map(c => (
                    <tr key={c.id} onClick={() => selectCustomerRow(c.id)} className="cursor-pointer hover:bg-[#795bf4]/8 active:bg-[#795bf4]/10 transition-colors">
                      ...
                      <td className="py-2 px-2"><div className="flex items-center gap-1">{c.channels.map(ch => <span key={ch}>{DIR_CHANNEL_ICON[ch]({ size: 15 })}</span>)}{c.online && <span className="ml-0.5 h-1.5 w-1.5 rounded-full bg-[#66dba3]" />}</div></td>
          ...
          <>
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-3">
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                <div className="rounded-lg border border-[#66dba3] bg-[#66dba3]/12 p-2.5"><p className="text-[10px] text-[#66dba3]">WhatsApp</p><p className="text-lg font-bold text-[#66dba3]">9</p></div>
                  ...
                  {filteredReviewLeads.map((lead) => (
                    <tr key={lead.id} className="hover:bg-[#795bf4]/8 transition-colors">
                      ...
                      <td className="py-2 px-2">
                        <span className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${lead.status === 'Reviewed' ? 'border-[#66dba3] bg-[#66dba3]/12 text-[#66dba3]' : lead.status === 'Needs Link' ? 'border-[#795bf4]/20 bg-[#795bf4]/10 text-[#5a3fe0]' : 'border-rose-200 bg-rose-50 text-rose-700'}`}>
                          {lead.status}
                        </span>
                      </td>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4 space-y-6">
          <section className="rounded-xl border border-zinc-200 bg-white shadow-sm p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-[10px] font-medium px-2 py-1 rounded-full border ${orderVol === 0 ? 'border-zinc-200 text-zinc-500 bg-zinc-100' : orderVol === 1 ? 'border-[#795bf4]/20 text-[#5a3fe0] bg-[#795bf4]/10' : 'border-[#66dba3] text-[#66dba3] bg-[#66dba3]/12'}`}>{statusLabel}</span>
            </div>
          ...
          <section className="rounded-xl border border-zinc-200 bg-white shadow-sm p-4">
            ...
            <div className="space-y-2">
              ...
              {txns.map(txn => {
                ...
                const sc = s === 'paid' ? 'border-[#66dba3] text-[#66dba3] bg-[#66dba3]/12' : (s === 'pending' || s === 'sent' || s === 'draft') ? 'border-[#795bf4]/20 text-[#5a3fe0] bg-[#795bf4]/10' : 'border-rose-200 text-rose-700 bg-rose-50';
