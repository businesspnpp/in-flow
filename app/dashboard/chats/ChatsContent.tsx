...
import {
  ...
  Download, FileText, Filter, Hash, Inbox, MessageSquare, MoreHorizontal, Paperclip, Phone,
...
export default function ChatsContent() {
  ...
  function handleGlobalSearch(v: string) {
    setGlobalQuery(v);
    setListSearch(v);
    setDirectorySearch(v);
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden h-full bg-slate-50">
      {/* Page header */}
      <div className="flex-shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 bg-white border-b border-zinc-200">
        <div>
          <h1 className="text-xl font-bold text-zinc-900">Chats</h1>
          <p className="text-xs text-zinc-500 mt-0.5">Find conversations, customers, and message context quickly.</p>
        </div>
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full h-9 rounded-lg border border-zinc-200 bg-zinc-50 pl-9 pr-3 text-sm text-zinc-700 placeholder:text-zinc-400 outline-none focus:border-blue-400 focus:bg-white transition"
          />
        </div>
      </div>

      {/* Four collapsible columns */}
      <div className="flex-1 flex overflow-x-auto overflow-y-hidden min-h-0">

        {/* Column 1: Inbox / chat list */}
        <CollapsiblePanel title="Inbox" icon={Inbox} expanded={panels.list} onToggle={() => togglePanel('list')} widthClass="w-80">
          ...
          <div className="divide-y divide-zinc-100">
            {filteredChats.map(contact => (
              <button key={contact.id} onClick={() => selectChat(contact.id)} className={`w-full flex items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-blue-50/60 ${activeContact === contact.id ? 'bg-blue-50/80' : ''}`}>
              ...
              <table className="w-full text-[11px] min-w-[480px]">
                ...
                <tbody className="divide-y divide-zinc-100">
                  {filteredCustomers.map(c => (
                    <tr key={c.id} onClick={() => selectCustomerRow(c.id)} className="cursor-pointer hover:bg-blue-50/50 transition-colors">
        ...
        <CollapsiblePanel title="Conversation" icon={MessageSquare} expanded={panels.thread} onToggle={() => togglePanel('thread')} widthClass="flex-1 min-w-[360px]">
          {!activeContact ? (
            ...
            <div className="h-full flex flex-col">
              ...
              <div key={activeContact} className="flex-1 min-h-0 overflow-y-auto px-4 py-4 flex flex-col gap-3">
        ...
        {/* Column 4: Customer profile / context */}
