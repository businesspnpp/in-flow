...
// Unified Dataset mirroring the new list design
      ...
      <div className="mx-auto w-full max-w-7xl space-y-6">
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-200 pb-5">
          ...
          <div className="flex items-center gap-3">
            <button className="rounded-lg bg-orange-600 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-orange-500 transition-colors">
              New Booking
            </button>
            <button className="rounded-lg bg-indigo-600 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-indigo-500 transition-colors">
              Export Data
            </button>
          </div>
        </header>

        {/* Workspace Context Filtering Tabs */}
            ...
            <tbody className="divide-y divide-zinc-200">
              {filteredRecords.map((record) => (
                <tr key={record.id} className="hover:bg-zinc-50/50 transition-colors">
                  ...
                  {/* Service Description and Subtext Line Cell */}
                  <td className="px-4 py-4">
                    <div className="font-medium text-zinc-900">{record.service}</div>
                    {record.subText && (
                      <div className="text-[11px] text-zinc-400 mt-0.5">{record.subText}</div>
                    )}
                  </td>

                  {/* Multi-line Date & Time Timestamp Cell */}
                  <td className="px-4 py-4 text-zinc-500 whitespace-nowrap">
                    {record.dateTime.split(' @ ').map((part, i) => (
                      <span key={i} className="block first:font-medium first:text-zinc-800 last:text-[11px] last:text-zinc-400 last:mt-0.5">
                        {part}
                      </span>
                    ))}
                  </td>

                  {/* Dynamic Custom Color Status Badge Cell */}
                  <td className="px-4 py-4 whitespace-nowrap">
                  ...
                  {/* Dense Financial Amount and Confirmation State Cell */}
                  <td className="px-4 py-4 whitespace-nowrap">
                    <div className="font-bold text-zinc-900">{record.payment}</div>
                    <div className={`text-[10px] font-medium mt-0.5 ${paymentTextClass(record.paymentStatus)}`}>
                      ({record.paymentStatus})
                    </div>
                  </td>

                  {/* Modular Action Options Stack Panel Cell */}
                  <td className="px-5 py-4 whitespace-nowrap text-right">
                    {record.actions.length > 0 ? (
                      <div className="inline-flex flex-col gap-1 w-28">
                      ...
                      <span className="text-zinc-300 text-xs italic pr-4">—</span>
                    )}
                  </td>
        ...
        {/* Bottom Horizontal Stats Widget Row matching Image layout exactly */}
        ...
        {/* Synchronized Workspace Meta Footer Bar */}
        <footer className="rounded-xl border border-zinc-200 bg-zinc-100/50 px-5 py-3 text-xs text-zinc-500 font-medium flex flex-wrap gap-x-6 gap-y-2 items-center justify-center sm:justify-start">
          <span>Conversations Synced: <strong className="font-bold text-zinc-800">1,248</strong></span>
          <span className="hidden sm:inline text-zinc-300">|</span>
          <span>Active Workflows: <strong className="font-bold text-zinc-800">8</strong></span>
          <span className="hidden sm:inline text-zinc-300">|</span>
          <span>Bookings Managed: <strong className="font-bold text-zinc-800">154</strong></span>
        </footer>
